...
NtupleMaker::NtupleMaker() : Processor("NtupleMaker") {
  ...
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "", "", m_DressedLepton,
...
void NtupleMaker::fillMissingEnergy(LCEvent* evt) {
  ...
  for (unsigned int i = 0; i < jet_vlc_R08_pt.size(); i++) {
    tmp0vec.SetPtEtaPhiE(jet_vlc_R08_pt.at(i), EtaFromTheta(jet_vlc_R08_theta.at(i)), jet_vlc_R08_phi.at(i),
                         jet_vlc_R08_e.at(i));
    fourvec -= tmp0vec;
  ...
  for (unsigned int i = 0; i < lep_dressed_pt.size(); i++) {
    tmp0vec.SetPtEtaPhiE(lep_dressed_pt.at(i), EtaFromTheta(lep_dressed_theta.at(i)), lep_dressed_phi.at(i),
                         lep_dressed_e.at(i));
    fourvec -= tmp0vec;
